...
# 递归函数
def extract_comments(bv, aid, response, comment_list, count):
    ...
    for comment in comments:
        ...
        if 'replies' in comment:
            ...
            count += 1
            count = extract_comments(bv, aid, {'data': {'replies': nested_comments}}, comment_list, count)
    return count
...
def handle_video(video_link):
    ...
    while True:
        ...
        if 'replies' not in response['data']:
            break
        count = extract_comments(bv, aid, response, comment_list, count)
